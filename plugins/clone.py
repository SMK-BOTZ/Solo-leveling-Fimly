...
async def delete_cloned_bot(client, message):
    ...
    try:
        if not message.text:
            await message.reply_text("Message text is empty. Please provide a valid bot token.")
            return

        bot_token_match = re.search(r'\d[0-9]{8,10}:[0-9A-Za-z_-]{35}', message.text, re.IGNORECASE)
        bot_token = bot_token_match.group(0) if bot_token_match else None

        if not bot_token:
            await message.reply_text("Invalid or missing bot token. Please provide a valid token.")
            return
        ...
        if cloned_bot:
            ...
            await message.reply_text(
                "**🤖 The cloned bot has been successfully removed from the list and its details have been deleted from the database. ☠️**"
            )
        else:
            await message.reply_text(
                "**⚠️ The bot token provided is not in the cloned list.**"
            )
    except Exception as e:
        logging.exception(f"Error while deleting cloned bot: {e}")
