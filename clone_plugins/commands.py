...
from clone_plugins.start_text import *
...
# Command to display the current start text with buttons
...
async def start(client, message):
    ...
    # If command doesn't have additional arguments, display buttons and default text
    if len(message.command) != 2:
        ...
        start_text = load_start_text(client.me.id)
        await message.reply_photo(
            photo=random.choice(PICS),  # Choose a random photo from the list
...
async def cb_handler(client: Client, query: CallbackQuery):
    if query.data == "close_data":
        ...
        # Edit the message media (random photo from the list)
        await query.message.edit_media(
            media=InputMediaPhoto(random.choice(PICS))
        )

        # Edit the text with the loaded start text and buttons
